...
export class AdminService {
  ...
  private defaultCmsContent() {
    return {
      hero: {
        ...
        description: 'زیباگر مناسب خود را پیدا کنید',
        ctaText: 'جستجو',
        ctaLink: '/search',
        badge: 'بیوتی‌جو',
      ...
      texts: {
        ...
        ctaDescription: 'زیباگر را انتخاب کنید و نوبت بگیرید.',
  ...
  async updateSiteBuilder(sections: unknown[], actorId?: string) {
    ...
    await this.audit(actorId, 'site_builder.draft.update', 'platform_setting', null, before, sections);
    return { sections };
  }
  ...
  async publishSiteCms(actorId?: string) {
    ...
    await this.audit(actorId, 'site_cms.publish', 'platform_setting', null, null, { publishedAt });
  ...
  async uploadSiteCmsImage(
    ...
    const ext =
      ...
      ['png', 'webp', 'gif', 'jpg', 'jpeg'].includes(extFromName) ? extFromName : 'bin';
    ...
    const contentType = mime.startsWith('image/') ? mime : `image/${ext}`;
    const storedKey = await this.storage.upload(key, raw, contentType);
    if (file.path) {
      try { await fs.unlink(file.path); } catch { /* ignore */ }
    }
    const url = this.storage.getPublicUrl(storedKey);
    await this.audit(actorId, 'site_cms.upload', 'media', null, null, { key: storedKey, slot: slotSafe, url });
  ...
  async listServiceCategories() {
    try {
      ...
      this.logger.error('listServiceCategories failed', e as Error);
  ...
  async deleteServiceCategory(id: string, actorId?: string) {
    ...
    await this.audit(actorId, 'catalog.category.deactivate', 'service_category', id, existing, { isActive: false });
    return { success: true };
  }
  ...
  async deleteCatalogService(id: string, actorId?: string) {
    ...
    await this.audit(actorId, 'catalog.service.deactivate', 'service', id, existing, { isActive: false });
    return { success: true };
  }
