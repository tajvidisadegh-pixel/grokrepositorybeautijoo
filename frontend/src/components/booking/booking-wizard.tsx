            ...
            <div className="space-y-2 border-t border-border pt-3">
              {!showAddOns ? (
                ...
                <>
                  ...
                  <button type="button" className="text-xs text-gray-muted" onClick={() => { setShowAddOns(false); setSelectedAddOnIds([]); }}>
                    رد کردن
                  </button>
      ...
      {step === 'datetime' && selected && (
        <Card className="mt-6 space-y-4">
          ...
          <div>
            ...
            {slotsLoading && <p className="text-sm text-gray">در حال بارگذاری...</p>}
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <Button variant="outline" className="flex-1" onClick={() => setStep('service')}>بازگشت</Button>
            <Button className="flex-1" disabled={!slotStart} onClick={goSummary}>ادامه</Button>
          </div>
        </Card>
      )}
      ...
      {step === 'summary' && selected && (
        <Card className="mt-6 space-y-4">
          ...
          <dl className="space-y-2 text-sm">
            <div className="flex justify-between gap-2"><dt className="text-gray">زیباگر</dt><dd>{professional.name}</dd></div>
            <div className="flex justify-between gap-2"><dt className="text-gray">خدمت</dt><dd>{selected.name}{selectedRuleLabel ? ` · ${selectedRuleLabel}` : ''}</dd></div>
            {selectedAddOnNames.length > 0 && (
              <div className="flex justify-between gap-2"><dt className="text-gray">اضافات</dt><dd>{selectedAddOnNames.join('، ')}</dd></div>
            )}
