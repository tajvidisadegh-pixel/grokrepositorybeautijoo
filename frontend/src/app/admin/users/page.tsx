...
export default function AdminUsersPage() {
  ...
  const sendNotify = async () => {
    ...
    try {
      ...
      setActionMsg(`اعلان برای ${res.notified} کاربر ارسال شد${notifySms ? ` (SMS: ${res.smsSent})` : ''}`);
      setNotifyOpen(false);
      setSelected(new Set());
    } catch (e) {
      setActionMsg(friendlyApiError(e));
                ...
                {(detail.auditLogs || []).length > 0 && (
                  <div>
                    ...
                    <ul className="space-y-1 text-xs text-gray">
                      {(detail.auditLogs || []).slice(0, 15).map((a: any) => (
                        <li key={a.id}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setNotifyOpen(false)}>
          ...
          >
            <h3 className="mb-4 text-lg font-bold">
              ارسال اعلان به {notifyTargetIds.length} کاربر
            </h3>
