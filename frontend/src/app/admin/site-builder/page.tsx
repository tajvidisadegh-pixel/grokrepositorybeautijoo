    ...
    <div className="min-h-screen bg-[#f0f0f1]" dir="rtl">
      ...
      {(error || message) && (
        <div className="mx-auto max-w-[1400px] space-y-2 px-3 pt-3">
          ...
          {message && (
            <div className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-[#2271b1]">
              {message}
            </div>
          )}
        </div>
      ...
      <div className="mx-auto grid max-w-[1400px] gap-0 lg:grid-cols-[240px_1fr_320px]">
                      ...
                      {hasBanner && layout === 'image-background' ? (
                        ...
                        <div className="grid gap-0 md:grid-cols-2">
                          <div className="flex flex-col justify-center bg-white p-6 text-right">
                            ...
                            {hero.subtitle && (
                              <p className="mt-1 text-base text-gray-800">{hero.subtitle}</p>
                            )}
                            {hero.description && (
                              <p className="mt-2 text-sm text-gray-600">{hero.description}</p>
                            )}
                          </div>
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img
                        ...
                        <div className="relative px-6 py-12 text-center">
                          {hero.badge && (
                            <p
                              className={`mb-2 text-xs font-semibold ${
                                forceLight ? 'text-orange-200' : 'text-[#FF6F61]'
                              }`}
                            >
                              {hero.badge}
                            </p>
                          ...
                          <p
                            className={`text-2xl font-bold ${
                              forceLight ? 'text-white' : 'text-[#0B2C4A]'
                            }`}
                          >
                          ...
                          {hero.subtitle && (
                            <p
                              className={`mt-1 text-lg ${
                                forceLight ? 'text-white/95' : 'text-gray-800'
                              }`}
                            >
                              {hero.subtitle}
                            </p>
                          ...
                          {hero.description && (
                            <p
                              className={`mx-auto mt-2 max-w-md text-sm ${
                                forceLight ? 'text-white/85' : 'text-gray-600'
                              }`}
                            >
                              {hero.description}
                            </p>
                ...
                if (id === 'categories' && features.showCategories !== false) {
                  return (
                    <div
                      ...
                      className={`cursor-pointer px-6 py-8 ${
                        selected === 'categories' ? 'ring-2 ring-inset ring-[#2271b1]' : ''
                      }`}
                    >
                      <div className="mb-3 flex items-center justify-between">
                        <span className="font-bold text-[#0B2C4A]">
                      ...
                      <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">
                        {[1, 2, 3, 4, 5, 6].map((n) => (
                          <div
                            key={n}
                            className="rounded-xl border bg-gray-50 py-4 text-center text-[11px] text-gray-400"
                          >
                            دسته
                          </div>
                        ))}
                ...
                if (id === 'featured' && features.showFeaturedProfessionals !== false) {
                  return (
                    <div
                      ...
                      className={`cursor-pointer px-6 py-8 ${
                        selected === 'featured' ? 'ring-2 ring-inset ring-[#2271b1]' : ''
                      }`}
                    >
                      <div className="mb-3 flex items-center justify-between">
                        <span className="font-bold text-[#0B2C4A]">
                      ...
                      <div className="flex gap-2 overflow-hidden">
                        {[1, 2, 3].map((n) => (
                          <div
                            key={n}
                            className="h-24 w-32 shrink-0 rounded-xl border bg-gray-50 text-center text-[11px] leading-[6rem] text-gray-400"
                          >
                            کارت
                          </div>
                        ))}
                ...
                if (id === 'cta' && features.showBottomCta !== false) {
                  return (
                    <div
                      ...
                      className={`cursor-pointer bg-[#0B2C4A] px-6 py-10 text-center text-white ${
                        selected === 'cta' ? 'ring-2 ring-inset ring-[#2271b1]' : ''
                      }`}
                    >
                      <p className="text-lg font-bold">{texts.ctaTitle || 'آماده رزرو هستید؟'}</p>
                      <p className="mt-1 text-sm text-white/80">
                        {texts.ctaDescription || 'زیباگر را انتخاب کنید و نوبت بگیرید.'}
                      </p>
                      <div className="mt-4 flex flex-wrap justify-center gap-2">
                        <span className="rounded-lg bg-[#FF6F61] px-3 py-1.5 text-xs">
                          {texts.ctaPrimaryText || 'شروع جستجو'}
                        </span>
                        <span className="rounded-lg border border-white/40 px-3 py-1.5 text-xs">
        ...
        <aside className="border-r border-[#dcdcde] bg-white lg:min-h-[calc(100vh-48px)]">
          <div className="border-b border-[#dcdcde] px-3 py-3">
            ...
            <p className="mt-0.5 text-sm font-medium text-[#1d2327]">
              {blocks.find((b) => b.id === selected)?.title}
            </p>
          </div>
          <div className="max-h-[calc(100vh-120px)] space-y-4 overflow-y-auto p-3">
            {selected === 'hero' && (
              <>
                ...
                <Field
                  label="متن placeholder جستجو"
                  value={hero.searchPlaceholder || ''}
                  onChange={(v) => patchHero({ searchPlaceholder: v })}
                />
                <Toggle
                  label="نمایش جستجو در بنر"
                  checked={features.showSearchInHero !== false}
                  onChange={(v) => patchFeatures({ showSearchInHero: v })}
                />
                <div className="rounded-lg border border-[#dcdcde] p-3">
                  <p className="mb-2 text-xs font-semibold text-[#1d2327]">رسانه بنر</p>
                  <MediaSlot
                    label="تصویر دسکتاپ"
                    url={hero.desktopImageUrl}
                    uploading={uploading === 'desktop'}
                    onUrl={(v) => patchHero({ desktopImageUrl: v })}
                    onFile={(f) => void onUpload('desktop', f)}
                    onClear={() => patchHero({ desktopImageUrl: null })}
                  />
                  <div className="mt-3">
                    <MediaSlot
                      label="تصویر موبایل"
                      ...
                      onUrl={(v) => patchHero({ mobileImageUrl: v })}
                      onFile={(f) => void onUpload('mobile', f)}
                      onClear={() => patchHero({ mobileImageUrl: null })}
                    />
                  </div>
                </div>
                <Range
                  label={`تاریکی روی تصویر (فقط برای خوانایی متن): ${overlayPct}%`}
                  value={overlayPct}
                  min={0}
                  max={80}
                  onChange={(v) => patchHero({ overlayOpacity: v })}
                />
                <p className="text-[11px] text-[#646970]">
                  برای بنر کاملاً واضح، عدد را نزدیک ۰ بگذارید. برای خوانایی متن روی تصویر شلوغ، ۳۰–۵۰ مناسب است.
                </p>
                <Range
                  label={`حداقل ارتفاع بنر: ${hero.minHeight ?? 420}px`}
                  value={Number(hero.minHeight ?? 420)}
                  min={280}
                  max={700}
                  onChange={(v) => patchHero({ minHeight: v })}
                />
                <Select
                  label="تراز متن"
                  value={hero.textAlign || 'center'}
                  onChange={(v) => patchHero({ textAlign: v as Hero['textAlign'] })}
                  options={[
                ...
                <Select
                  label="رنگ متن"
                  value={hero.textColor || 'auto'}
                  onChange={(v) => patchHero({ textColor: v as Hero['textColor'] })}
                  options={[
                    { value: 'auto', label: 'خودکار (روشن روی تصویر)' },
                    { value: 'light', label: 'روشن (سفید)' },
            ...
            {selected === 'categories' && (
              <>
                <Toggle
                  label="نمایش بخش دسته‌بندی‌ها"
                  checked={features.showCategories !== false}
                  onChange={(v) => patchFeatures({ showCategories: v })}
                />
                <Field
                  label="عنوان بخش"
            ...
            {selected === 'featured' && (
              <>
                <Toggle
                  label="نمایش زیباگرهای برتر"
                  checked={features.showFeaturedProfessionals !== false}
                  onChange={(v) => patchFeatures({ showFeaturedProfessionals: v })}
                />
                <Field
                  label="عنوان بخش"
            ...
            {selected === 'cta' && (
              <>
                <Toggle
                  label="نمایش نوار CTA"
                  checked={features.showBottomCta !== false}
                  onChange={(v) => patchFeatures({ showBottomCta: v })}
                />
            ...
            {selected === 'layout' && (
              <>
                <p className="text-xs text-[#646970]">
                  ترتیب بخش‌ها را با دکمه‌های بالا/پایین تنظیم کنید. غیرفعال کردن بخش آن را از صفحه حذف می‌کند.
                </p>
                <div className="space-y-2">
                  {sections.map((s, i) => (
                    <div
                      key={s.id}
                      className="flex items-center justify-between rounded-lg border border-[#dcdcde] bg-[#f6f7f7] px-2 py-2"
                    >
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          ...
                          className="rounded bg-white px-1.5 py-0.5 text-[11px] shadow-sm disabled:opacity-40"
                        >
                          ↑
                        </button>
                        <button
    ...
    <label className="block text-xs">
      ...
      {multiline ? (
        <textarea
          ...
          rows={3}
          className="w-full rounded border border-[#8c8f94] px-2 py-1.5 text-sm outline-none focus:border-[#2271b1] focus:ring-1 focus:ring-[#2271b1]"
        />
      ) : (
        <input
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className="h-9 w-full rounded border border-[#8c8f94] px-2 text-sm outline-none focus:border-[#2271b1] focus:ring-1 focus:ring-[#2271b1]"
        />
      )}
    </label>
  );
  ...
  return (
    <label className="block text-xs">
      ...
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="h-9 w-full rounded border border-[#8c8f94] bg-white px-2 text-sm outline-none focus:border-[#2271b1]"
      >
        {options.map((o) => (
  ...
  return (
    <div>
      ...
      {url ? (
        <div className="relative mb-2 overflow-hidden rounded-lg border border-[#dcdcde]">
          ...
          <button
            type="button"
            onClick={onClear}
            className="absolute left-2 top-2 rounded bg-black/60 px-2 py-0.5 text-[11px] text-white"
          >
            حذف
          </button>
        </div>
      ) : (
        <div className="mb-2 flex h-28 items-center justify-center rounded-lg border border-dashed border-[#c3c4c7] bg-[#f6f7f7] text-[11px] text-[#646970]">
      ...
      <input
        ...
        accept="image/*"
        disabled={uploading}
        className="mb-1 block w-full text-[11px]"
      ...
      <input
        ...
        className="mt-1 h-8 w-full rounded border border-[#8c8f94] px-2 text-xs"
      />
    </div>
  );
}
