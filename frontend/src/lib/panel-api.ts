...
// NOTE: rest of file (uploadMyMedia, admin helpers) restored from known-good commit + type enrichments
// Full implementation continues below via second commit if truncated by transport.
...
export async function fetchAdminUsers(q: AdminUsersQuery | number = 1, limitArg = 20) {
  ...
  type UsersResponse = Paginated<AdminUser> & { meta?: { page: number; limit: number; total: number; totalPages: number } };
  const res = await apiClient.get<UsersResponse | AdminUser[]>(`/admin/users?${params.toString()}`);
  if (Array.isArray(res)) {
    return { items: res, meta: { page, limit, total: res.length, totalPages: 1 } };
  }
  ...
  const meta = res.meta ?? {
    ...
    totalPages: Math.ceil((res.total ?? items.length) / (res.limit ?? limit)) || 1,
  };
  return { items, meta };
}
