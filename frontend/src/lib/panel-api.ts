...
export async function fetchAdminUsers(q: AdminUsersQuery | number = 1, limitArg = 20) {
  ...
  const res = await apiClient.get<Paginated<AdminUser> & { meta?: { page: number; limit: number; total: number; totalPages: number } }>(
  ...
  const meta = (res as any).meta || {
    ...
    totalPages: Math.ceil(((res as any).total ?? items.length) / limit) || 1,
  };
