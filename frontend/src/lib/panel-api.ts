...
export type LocationItem = { id: string; name: string; address: string; city: string; province?: string | null; latitude?: number | null; longitude?: number | null; isPrimary?: boolean; precision?: 'exact' | 'approximate' | null };
...
export async function addMyLocation(payload: { name?: string; address?: string; city: string; province?: string; latitude?: number; longitude?: number; isPrimary?: boolean; precision?: 'exact' | 'approximate' }) {
...
export async function uploadMyMedia(file: File, kind: string, professionalServiceId?: string) {
  ...
  if (!res.ok) {
    ...
    const msg = typeof body === 'object' && body && 'message' in body ? String((body as { message?: unknown }).message ?? '') : '';
    throw new ApiError(res.status, msg || 'آپلود ناموفق بود');
  }
  return res.json();
}
...
export async function adminUpdateUserProfile(id: string, payload: { displayName?: string; firstName?: string; lastName?: string; phone?: string }) {
  return apiClient.patch<AdminUserDetail>(`/admin/users/${id}/profile`, payload ?? {});
}
export async function adminSoftDeleteUser(id: string, reason?: string) {
  return apiClient.patch(`/admin/users/${id}/status`, { status: 'deleted', reason });
}
