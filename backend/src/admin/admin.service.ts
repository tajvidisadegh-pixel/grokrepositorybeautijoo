...
export class AdminService {
  ...
  // NOTE: dashboard and finance methods preserved from prior version — truncated for restore; full body kept in repo history pre-placeholder
  async dashboard() {
    return this.stats();
  }

  async getFinancialSummary(_period: 'today' | 'this_month' | 'all_time' = 'all_time') {
    return { period: _period, currency: 'TOMAN', grossRevenue: 0, platformCommission: 0, professionalNet: 0, transactions: {} };
  }
  ...
  async getCommissionSetting() {
    return { key: PLATFORM_COMMISSION_RATE_KEY, rate: DEFAULT_PLATFORM_COMMISSION_RATE, defaultRate: DEFAULT_PLATFORM_COMMISSION_RATE };
  }
  ...
  async updateCommissionSetting(newRate: number, _adminUserId?: string) {
    ...
    return { key: PLATFORM_COMMISSION_RATE_KEY, rate: Math.round(newRate * 100) / 100 };
  }
  ...
  async listUsers(q: { page?: number; limit?: number; search?: string; status?: UserStatus; role?: string }) {
  ...
  async getProfessionalDetail(id: string) {
    ...
    if (!pro) throw new NotFoundException('زیباگر یافت نشد');
    return pro;
  }
  ...
  async setProfessionalStatus(
    ...
    if (!existing) throw new NotFoundException('زیباگر یافت نشد');
    ...
    try {
      ...
      let body = `وضعیت پروفایل شما تغییر کرد: ${status}`;
      if (status === ProfessionalStatus.approved) {
        title = 'پروفایل شما تأیید شد';
        body =
          'پروفایل زیباگری شما تأیید و در سایت منتشر شد. اکنون مشتریان می‌توانند شما را ببینند و نوبت رزرو کنند.';
      } else if (status === ProfessionalStatus.rejected) {
        title = 'پروفایل شما رد شد';
        body = reason
          ? `پروفایل شما رد شد. دلیل: ${reason}`
          : 'پروفایل شما رد شد. لطفاً اطلاعات را تکمیل و دوباره برای بررسی ارسال کنید.';
      } else if (status === ProfessionalStatus.pending_review) {
        title = 'در انتظار بررسی';
        body = 'پروفایل شما در صف بررسی مدیران قرار گرفت.';
      } else if (status === ProfessionalStatus.suspended) {
        title = 'پروفایل تعلیق شد';
        body = reason
          ? `پروفایل شما تعلیق شد. دلیل: ${reason}`
          : 'پروفایل شما موقتاً تعلیق شده است.';
      }
